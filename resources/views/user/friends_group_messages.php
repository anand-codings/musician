                ...
                <div class="chat_usersidebar_wrap">
                    <div class="chat_alluser_wrap">
                        <div class="head d-flex">
                            <div>
                                <h5 class="font-weight-bold text_darkblue">Group Messages
                                    <span class="new_messages_label"><span><?= $unread ?></span> New</span>
                                </h5>
                            </div>
                            <div class="ml-auto">
                                        <span class="new_chat_btn">
                                            <i data-toggle="modal" data-target="#modal_friends_message"
                                               class="fa fa-edit"></i>
                                        </span>
                            </div>
                        </div>
                        <!-- Followers modal Start -->
                        <div class="modal fade" id="followers_modal" tabindex="-1" role="dialog"
                             aria-labelledby="followers_modal" aria-hidden="true">
                                                        ...
                                                        <div class="media align-items-center">
                                                            ...
                                                            <div class="media-body">
                                                                <div class="d-flex flex-column flex-sm-row">
                                                                    <div class="mb-2">
                                                                        <a href="#"
                                                                           class="u_name"><?= $ntchated->first_name . ' ' . $ntchated->last_name; ?></a>
                                                                    ...
                                                                    <div class="d-flex align-items-center  ml-sm-auto">
                                                                        <div class="following_status">
                                                                            <a onclick="hideModal('followers_modal', 'modal_new_messages<?= $ntchated->id ?>')"
                                                                               href="#" class="btn_message"> <i
                                                                                        class="s_icon ic_message white"></i>
                                                                                Message </a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                        ...
                        <?php foreach ($not_chated as $ntchated) { ?>
                            <!-- Message modal Start -->
                            <div class="modal fade" id="modal_new_messages<?= $ntchated->id ?>" tabindex="-1"
                                 role="dialog" aria-labelledby="add_affiliation" aria-hidden="true">
                                <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h6 class="modal-title text-black" id="exampleModalLabel">New message To
                                                <span class="text_maroon"> <?= $ntchated->first_name . ' ' . $ntchated->last_name; ?> </span>
                                            </h6>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <i class="fas fa-times-circle"></i>
                                            </button>
                                        ...
                                        <div class="modal-body">
                                            <form>
                                                <div class="row">
                                                    <div class="col-12">
                                                        <div class="form-group mb-0">
                                                            <textarea id="message<?= $ntchated->id ?>"
                                                ...
                                                <div class="mt-2">
                                                    <button type="button"
                                                            onclick="sendMessagePopUp('<?= $ntchated->id ?>')"
...
//                                            }
                                    if(!empty($g_chat->title)){
                                        $name = $g_chat->title;
                                    } else {
                                        $name = $g_chat->receiver[0]->first_name. ' ' .$g_chat->receiver[0]->last_name;
                                    }
                                    $profile_url = '#';
//                                            if ($other_user->is_active) {
//                                                $name = $other_user->first_name . ' ' . $other_user->last_name;
...
//                                            $chat_type_sidebar = 'u';
                                    ...
                                    foreach ($other_user_imgs as $key => $value) {
                                        if($value->getMemberDetail->id != $current_id){
                                            if ($key == 3) {
                                                break;
                                            } else {
...
//                                            }
                                    ?>
                                <li data-id="<?= $g_chat->id ?>" id="single_chat_user<?= $g_chat->id ?>"
                                    class="chat_user_listing <?php if ($i == 1) { ?> active <?php } ?>">
                                    <div class="list_outer_wrap d-flex">
                                    <div class="custom-control custom-checkbox">
                                        <?php
                                        if ($g_chat->admin_id == $current_id) {
                                            ?>
                                            <input data-id="<?= $g_chat->id ?>" type="checkbox" name=""
                                                   class="custom-control-input single_chat_list"
                                                   id="user_select<?= $g_chat->id ?>">
                                            <label class="custom-control-label no-top-padding txt_left"
                                                   for="user_select<?= $g_chat->id ?>"></label>
                                            <?php
                                        }
                                        ?>
                                    </div>
                                <a href="#" id="chat_on_left_menu<?= $g_chat->id ?>"
                                   onclick="getOtherChat('chat_id_<?= $g_chat->id ?>', this, '<?= $g_chat->id ?>', '<?= $name ?>', '<?= $other_user->id ?>', '<?= $profile_url ?>', '<?php echo $js_images_string ?>', '0', '10', '<?= $other_user->is_active ?>', '<?= $chat_type_sidebar ?>', '<?= $chat_type_id_siderbar ?>')">
                                    <div class="d-flex">
                                    ...
                                    <span class="bg_image_round user_active_status second-group-image"
                                        ...
                                        <?php }
                                        if (!empty($other_images[1])) { ?>
                                            <span class="bg_image_round user_active_status second-group-image"
                                                  style="background-image: url(<?= $other_images[1] ?>); ">

                                                                    </span>
                                        <?php }
                                        if (!empty($other_images[2])) { ?>
                                            <span class="bg_image_round user_active_status second-group-image"
                                        ...
                                        <div class="info">
                                            ...
                                            <div class="msg">
                                                ...
                                                //                                                                                                                            print_r($g_chat->lastMessage); exit;
                                                if (!empty($g_chat->lastMessage->message)) {
                                                    ...
                                                    if (!empty($g_chat->lastMessage->message)) {
                                                        if (strpos($g_chat->lastMessage->message, 'ifram') !== false) {
                                                            ...
                                                            $add_dot = substr($g_chat->lastMessage->message, 0, 65) . $add_dot;
                                                            echo $add_dot;
                                                        }
                                                    }
                                                }
                ...
                <div class="message_box_wrap">
                    ...
                    <div class="chat_header_box">
                        ...
                        <div class="chat_header d-flex align-items-center clearfix">
                            ...
                            foreach ($latest_chat->receiver as $key => $value) {
                            ...
                            <div class="media align-items-center">
                                ...
                                <div class="media-body line-height-13">
                                    <a id="active_user_name" href="<?= $profile_url ?>"
                                       class="text_darkblue font-18 font-weight-bold"><?= $name ?></a>
                                    <div id="online_section" <?php if ($other_message_user->is_online != 1) { ?> style="display: none" <?php } ?>
...
//                                            print_r($message); exit;
                                    ...
                                    if ($chat_count == 1) {
                                        ?>
                                        <input type="hidden" name="chat_id_scroll" id="chat_id_scroll"
                                               value="<?= $message->chat_group_id ?>">
                                        <?php
                                    }
                                            ...
                                            <div class="d-flex flex-row-reverse">
                                                <figure>
                                                    ...
                                                    if ($message->message_type == 'g' && $message->group->admin_id == $current_id) {
                                                        ...
                                                        ?>
                                                        <span class="bg_image_round"
                                                              onclick="location.href = '<?= asset('group_time_line/' . $message->group_id) ?>'"
                                                              style="background-image: url(<?php echo $pic ?>)"></span>
                                                        <?php
                                                    ...
                                                    <?php } else { ?>
                                                        ...
                                                        <span class="bg_image_round"
                                                              onclick="location.href = '<?= asset('profile_timeline/' . $current_id) ?>'"
                                                              style="background-image: url(<?php echo $current_photo ?>)"></span>
                                                    <?php } ?>
                                                </figure>
                                                <div class="chat_body">
                                                    <?php if ($message->message_type == 'g' && $message->group->admin_id == $current_id) { ?>
                                                        <div onclick="location.href = '<?= asset('group_time_line/' . $message->group->id) ?>'"
                                                             style="cursor: pointer;"
                                                             class="font-weight-bold text_darkblue text-right"><?= $message->group->name ?></div>
                                                    <?php } elseif ($message->message_type == 'a' && $message->accompanist->admin_id == $current_id) { ?>
                                                        <div onclick="location.href = '<?= asset('accompanist_time_line/' . $message->accompanist->id) ?>'"
                                                             style="cursor: pointer;"
                                                             class="font-weight-bold text_darkblue text-right"><?= $message->accompanist->name ?></div>
                                                    <?php } elseif ($message->message_type == 's' && $message->studio->admin_id == $current_id) { ?>
                                                        <div onclick="location.href = '<?= asset('teaching_studio_time_line/' . $message->studio->id) ?>'"
                                                             style="cursor: pointer;"
                                                             class="font-weight-bold text_darkblue text-right"><?= $message->studio->name ?></div>
                                                    <?php } else { ?>
                                                        <div onclick="location.href = '<?= asset('profile_timeline/' . $current_id) ?>'"
                                                             style="cursor: pointer;"
                                                             class="font-weight-bold text_darkblue text-right"><?= $current_name ?></div>
                                                    <?php } ?>
                                                    <div class="chat_txt highlight">
                                                        <?php if ($message->file_type && $message->file_type == 'image') { ?>
                                                            <div class="uploaded_image">
                                                                <a href="<?= asset('public/images/' . $message->file_path); ?>"
                                                        ...
                                                        <?php }
                                                        if ($message->file_type && $message->file_type == 'pdf') { ?>
                                                            <div class="uploaded_image">
                                                        ...
                                                        if ($message->file_type && $message->file_type == 'mp3') { ?>
                                                            <div class="uploaded_image">
                                                                <audio controls=""
                                                                       src="<?= asset('public/images/' . $message->file_path) ?>"></audio>
                                                        ...
                                                        <?php }
                                                        if ($message->file_type && $message->file_type == 'video') { ?>
                                                            <div class="uploaded_image">
                                                                <a href="<?= asset('public/videos/' . $message->file_path); ?>"
                                                                   data-fancybox="bigbuckbunny">
                                                                    <video width="320" height="240" controls
                                                                           src="<?= asset('public/videos/' . $message->file_path); ?>"></video>
                                                                </a>
                                                                <a href="<?= asset('download_file/' . $message->id) ?>">
                                                                    <!--<span class="image_download_btn"></span>-->
                                                                </a>
                                                                ...
                                                                <div class="message_dropdown_wrap">
                                                                    <?php if ($message->message) { ?>
                                                                        <input style="display: none" type="text"
                                                                               ...
                                                                               id="copy_<?= $message->id ?>">
                                                                        <a onclick="copyMessage('<?= $message->id ?>')"
                                                                           href="javascript:void(0)">Copy</a>
                                                                    <?php } ?>
                                                                    <a onclick="deleteMessage('<?= $message->id ?>', '<?= $message->chat_id ?>')"
                                        ...
                                        <li id="single_message<?= $message->id ?>">
                                            <div class="d-flex">
                                                <figure>
                                                    ...
                                                    <span class="bg_image_round"
                                                          onclick="location.href = '<?= $profile_url ?>'"
                                                          style="background-image: url(<?php echo $profile_pic ?>)"></span>
                                                </figure>
                                                <div class="chat_body">
                                                    <div onclick="location.href = '<?= $profile_url ?>'"
                                                         style="cursor: pointer;"
                                                         class="font-weight-bold text_darkblue"><?= $name ?></div>
                    ...
                    <div class="chat_footer_box">
                        <div style="display: none" class="files_upload_box">
                            <div class="d-flex flex-row-reverse">
                                <div class="uploaded_file">
                                    ...
                                    <img id="tiny-icon-spacer" src="<?= asset('userassets/images/spacer.png'); ?>"
                                         class="spacer">
                                    <div id="tiny-icon" class="image"
                                         style="background-image: url(<?= asset('userassets/images/groupimage1.jpg'); ?>);"></div>
                                    <video style="max-width: 75px; " id="tiny-video"
                        ...
                        <?php if (count($messages) > 0) { ?>
                            <div class="chat_footer"
                                 id="chat_area_messages" <?php if (!$other_message_user->is_active) { ?> style="display: none" <?php } ?>>
                                <textarea onkeyup="sendMessage(event, this, '0')" id="messagetext"
                                          placeholder="Write here your message.." class="form-control"></textarea>
                                <div class="btn_wrap">
                                    <div class="d-flex align-items-center">
                                        <div class="file_attachment">
                                            <input type="file" id="attachment"
                                                   accept="image/*,video/*,.mp3,.pdf,.dcx,.doc,.docm,.docx,.csv,.xlsx">
                                            <span class="icon-attachment"></span>
                                        </div>
                                        <button onclick="sendMessage(event, this, '1')" type="button"
                                                class="btn btn-round btn-gradient btn-xl text-semibold"/>
                                        <span>Send</span><i class="icon-arrow-right"></i></button>
                                    </div>
                                </div>
                            </div>
                            <div id="private_user" <?php if ($other_message_user->is_active) { ?> style="display: none" <?php } ?>>
                                You can't reply to this conversation.
                            </div>
                        <?php  }
                                else { ?>

                        <div class="chat_footer" id="chat_area_messages_first_time" style="<?=  ($other_message_user) ? '' : 'display: none' ?>">
...
<?php if (Auth::user()) { ?>
    <!-- Message modal Start -->
    <div class="modal fade" id="modal_friends_message" tabindex="-1" role="dialog" aria-labelledby="add_affiliation"
         aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title text-black" id="exampleModalLabel">New message To Friends<span
                ...
                <div class="modal-body">
                    <form>
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-group">
                                    <select type="text" name="friends[]" class="form-control"
                                            id="bulk-messages<?= $current_id ?>" style="width: 100%"
                                            multiple="multiple">
                                        <?php
                                        if (!empty($friends)) {
                                            foreach ($friends as $key => $friend) {
                                                ...
                                                <option <?= ($key == '0') ? 'selected=""' : '' ?>
                                                        value="<?= $friend->getFriendDetail->id ?>"> <?= $friend->getFriendDetail->first_name . ' ' . $friend->getFriendDetail->last_name ?> </option>
                                                <?php
                                            }
                                        }
                                        ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea id="message<?= $current_id ?>" class="form-control h_140"
                                              placeholder="Write a message"></textarea>
                                    <div class="font-14 text-danger text-right"></div>
                                </div>
                                <div class="form-group ">
                        ...
                        <div class="mt-2 text-center">
                            <button type="button" onclick="sendMessageToFriends('<?= $current_id ?>')"
...
<script>
    $(document).ready(function () {
        var old_id = $('#chat_type_id').val();
        $('.chat_box_wrapper').addClass('chat_box_wrapper_' + old_id);
    });
    var url_chat_id = "<?=(isset($_GET['chat_group_id']) && $_GET['chat_group_id']) ? $_GET['chat_group_id'] : ''?>";
    if (url_chat_id) {
...
//                            myCustomFn(this);
    ...
    socket.on('groupmessage_send', function (data) {
        var chat_group_id = $('#chat_type_id').val();
//            if(data.other_id != current_id){
        if (data.user_id == current_id && data.to_be_show == 'u') {
            ...
            if (li_exists == '') {
                ...
                chat_li += '</div>' +
                    '<a href="#" id="chat_on_left_menu' + data.chat_id + '" onclick="getOtherChat(\'chat_id_' + data.chat_id + '\', this, \'' + data.chat_id + '\', \'' + data.chat_name + '\', \'' + data.other_id + '\',\' # \', \'' + data.group_member_profile_images + '\',\'0\',\'10\',\'1\',\'u\',\'' + data.chat_id + '\')">' +
                    '<div class="d-flex">' +
                    '<div class="user_images_profile">';
                ...
                // chat_li += '<span class="bg_image_round user_active_status" style="background-image: url(' + data.photo + ') ">' +
                ...
                if(count_li == 0) {
                    $('#chat_on_left_menu' + data.chat_id).trigger('click');
                    $('.li_header_chat').show();
                    $('#chat_area_messages_first_time').show();
                }
            } else {
                if (chat_group_id == data.chat_id) {
                    $('#chat_on_left_menu' + data.chat_id + ' .msg').html(data.message);
                } else {
                    $('#chat_on_left_menu' + data.chat_id + ' .msg').html('<span class="active_status" style="background-color:blue;"></span>' + data.message);
                    $('.new_messages_label span').html(parseInt($('.new_messages_label span').html(), 10) + 1);
                }
            }
            ...
            if (/\S/.test(message)) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        //                                    $('.chat_box_wrapper .chat').append(result.append);
                        //                                    $('.chat_box_wrapper').mCustomScrollbar("scrollTo", 'bottom');
                        socket.emit('message_group_send', {
                            "user_id": otherid,
                            "other_id": '<?php echo $current_id; ?>',
                            "other_name": '<?php echo $current_user->first_name . ' ' . $current_user->last_name; ?>',
                            "photo": '<?php echo $current_photo; ?>',
                ...
                if (/\S/.test(message)) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            //
                            //                                                                   $('.chat_box_wrapper').mCustomScrollbar("scrollTo", 'bottom');
                             console.log(result.group_member_images);
