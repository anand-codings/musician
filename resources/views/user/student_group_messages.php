...
<html lang="en">
    ...
    <body>
        ...
        <input type="hidden" name="studio_chat_id" id="studio_chat_id" value="<?= $studio->id ?>">
                        ...
                        <div class="chat_usersidebar_wrap">
                            <div class="chat_alluser_wrap">
                                <div class="head d-flex">
                                    <div>
                                        <h5 class="font-weight-bold text_darkblue">Studio Messages
                                            <span class="new_messages_label"><span><?= $unread ?></span> New</span>
                                        </h5>
                                    </div>
                                    <div class="ml-auto">
                                        <span class="new_chat_btn">
                                            <i data-toggle="modal" data-target="#student_group_first_message_modal" class="fa fa-edit"></i>
                                        </span>
                                    </div>
                                </div>
                                
                                <!-- student message modal from messenger -->
                                    <?php if (Auth::user()) { ?>
                                    
                                    <div class="modal fade" id="student_group_first_message_modal" tabindex="-1" role="dialog" aria-labelledby="add_affiliation" aria-hidden="true">
                                        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h6 class="modal-title text-black" id="exampleModalLabel">New message To Studio Students <span class="text_maroon"> <?= $value['first_name'] . ' ' . $value['last_name']; ?> </span></h6>
...
?>
                                                                    ...
                                                                    <span class="bg_image_round user_active_status second-group-image"
                                                                          style="background-image: url(<?= getUserImage($value->getMemberDetail->photo, $value->getMemberDetail->social_photo, $value->getMemberDetail->gender) ?>); ">
                                                                 </span>
...
//                                                                    $pic = asset('public/images/' . $message->studio->pic);
                                                                ...
                                                                <span class="bg_image_round" onclick="location.href = '<?= asset('profile_timeline/' . $current_id) ?>'" style="background-image: url(<?php echo $pic ?>)"></span>
                                                                <?php
                                                        ...
                                                        <div class="chat_body">
                                                            ...
                                                            <?php } elseif ($message->message_type == 's' && $message->studio->admin_id == $current_id) { ?>
                                                                <div onclick="location.href = '<?= asset('teaching_studio_time_line/' . $message->studio->id) ?>'" style="cursor: pointer;" class="font-weight-bold text_darkblue text-right"><?= $current_name ?></div>   
                                                            <?php } else { ?>
                                                ...
                                                $name = 'Private User';
                                                $profile_url = '#';
//                                                if ($other_message_user->is_active && $message->message_type == 's') {
//
...
//                                                }
    ...
    <script>
        ...
        function sendMessageToStudents(otherid) {
                ...
                if(isEmpty(title))
                {
                   $('#showSuccess').html('Please add Group title !').fadeIn().fadeOut(5000);
                   return false;
                }
                if(isEmpty(message))
                {
                   $('#showSuccess').html('Please add Group message !').fadeIn().fadeOut(5000);
                   return false;
                }
                    ...
                    if (/\S/.test(message)) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                $('#student_group_first_message_modal').modal('hide');
                                //empty modal fields 
...
//                                $('#group_bulk_messages' + otherid).val();
                               ...
                               recivers.push('<?=$current_id?>');
                                ...
                                //refresh page after creating new group
                                // location.reload();
                                    }
                                });
                            }
        ...
        function sendMessage(e, obj, to_be_send) {
            ...
            if (e.keyCode === 13 || to_be_send == 1) {
                ...
                if (message || files) {
                    ...
                    var chat_group_id = $('#studio_chat_id').val();
//                    console.log(chat_type_id);
//                    return false;
                    ...
                    if (chat_type == 's') {
                        var chat_url = '<?= asset('student_studio_messages') ?>' + '/' + chat_group_id;
                    }
                    if (chat_type == 'u') {
                    ...
                    data.append('message_type', 's');
                    data.append('type_id', chat_group_id);
                    data.append('chat_group_id', chat_type_id);
                    data.append('_token', '<?= csrf_token() ?>');
                    $('#messagetext').val('');
                    if (/\S/.test(message) || files) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo asset('add_student_message_from_messenger'); ?>",
                            data: data,
                            processData: false,
                            contentType: false,
                            success: function (data) {
...
//            if(data.other_id != current_id){
                ...
                if (data.user_id == current_id && data.to_be_show == 's') {
                    var group_member_images_array =data.group_member_profile_images.split(',');
                    console.log(group_member_images_array);
                    other_id = $('#other_user_chat_id').val();
                    ...
                    if (li_exists == '') {
                        ...
                        chat_li += '</div>' +
                                '<a id="chat_on_left_menu' + data.chat_id + '"  href="#" onclick="getOtherChat(\'chat_id_' + data.chat_id + '\', this, \'' + data.chat_id + '\', \'' + data.chat_name + '\', \'' + data.other_id + '\',\' # \', \'' + data.group_member_profile_images + '\',\'0\',\'10\',\'1\',\'u\',\'' + data.chat_id + '\')">' +
                                '<div class="d-flex">' +
                                '<div class="user_images_profile">';
...
//            }
        ...
        function getOtherChat(id, ele, chat_id, other_name, other_id, url, image, skip, take, is_active, type = '', type_id = '') {
            var group_member_images_array= image.split(',');
            console.log(group_member_images_array);

            var old_id =$('#studio_chat_id').val();
