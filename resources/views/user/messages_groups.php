...
$chat_type = 'g';
$type_id = '';
$group_type_id = $group->id;

if ($latest_chat && $latest_chat->type == 'g') {
    $chat_type = 'g';
    $group_type_id = $latest_chat->type_id;
...
<input type="hidden" name="group_id" id="group_id" value="<?= $group_type_id ?>">
<input type="hidden" id="chat_type" value="<?= $chat_type ?>">
<input type="hidden" id="chat_type_id" value="<?= $type_id ?>">
<input type="hidden" id="is_chat_group_exit" value="<?= $other_message_user ? 'true' : 'fasle' ?>">
                ...
                <div class="chat_usersidebar_wrap">
                    <div class="chat_alluser_wrap">
                        <div class="head d-flex">
                            <div>
                                <h5 class="font-weight-bold text_darkblue">Event Messages
                                    <span class="new_messages_label"><span><?= $unread ?></span> New</span>
                                </h5>
                            </div>
                            <div class="ml-auto">
                                        <span class="new_chat_btn">
                                            <i data-toggle="modal" data-target="#friend_group_messages"
                                               class="fa fa-edit"></i>
                                        </span>
                            </div>
                        </div>
                                            ...
                                            <div class="d-flex flex-row-reverse">
                                                <figure>
                                                    ...
                                                    <span class="bg_image_round"
                                                          onclick="location.href = '<?php echo asset('profile_timeline/' . $current_id) ?>'"
...
<script>
    $(document).ready(function () {
        ...
        var url_chat_id = "<?=(isset($_GET['chat_event_id']) && $_GET['chat_event_id']) ? $_GET['chat_event_id'] : ''?>";
        if (url_chat_id) {
...
//                            myCustomFn(this);
    ...
    function sendMessage(e, obj, to_be_send) {
        if (e.keyCode === 13 || to_be_send == 1) {
            ...
            if (message || files) {
                ...
                if (chat_type == 'a') {
                    chat_url = '<?= asset('accompanist_messages') ?>' + '/' + chat_type_id;
                }
                if (chat_type == 'g') {
    ...
    socket.on('groupmessage_send', function (data) {
        var chat_group_id = $('#group_chat_id').val();
        if (data.user_id == current_id && data.to_be_show == 'g') {
            ...
            if (li_exists == '') {
                ...
                chat_li += '</div>' +
                    '<a id="chat_on_left_menu' + data.chat_id + '" href="#" onclick="getOtherChat(\'chat_id_' + data.chat_id + '\', this, \'' + data.chat_id + '\', \'' + data.chat_name + '\', \'' + data.other_id + '\',\' # \', \'' + data.group_member_profile_images + '\',\'0\',\'10\',\'1\',\'g\',\'' + data.chat_id + '\')">' +
                    '<div class="d-flex">' +
                    '<div class="user_images_profile">';
...
//            }
    ...
    function getOtherChat(id, ele, chat_id, other_name, other_id, url, image, skip, take, is_active, type = '', type_id = '') {
        ...
        if ($('#chat_on_left_menu' + chat_id + ' .msg').find('.active_status').length > 0) {
            if(parseInt($('.new_messages_label span').html(),10) > 0 ) {
                $('.new_messages_label span').html(parseInt($('.new_messages_label span').html(), 10) - 1);
            }
        }
...
<?php if ($current_user) { ?>
    <script>
        function sendMessageToFriends(otherid) {
            ...
            if (isEmpty(title)) {
                $('#showErrorAll').html('Please add Group title !').fadeIn().fadeOut(5000);
                return false;
            }
            if (isEmpty(message)) {
                $('#showErrorAll').html('Please add Group message !').fadeIn().fadeOut(5000);
                return false;
            }
            ...
            if (message) {
                ...
                if (/\S/.test(message)) {
                    ...
                    $.ajax({
                        ...
                        contentType: false,
                        success: function (data) {
                            $('#friend_group_messages').modal('hide');
                            counter++;
                            if (counter == 1) {
                                $('#showSuccess').html('Message Send Successfully !').fadeIn().fadeOut(5000);
                            }
                            ...
                            $.each(recivers, function (index, item) {
                                ...
                                if (item != <?=$current_id?>) {
                                    socket.emit('notification_get', {
                                        ...
                                        "url": '<?= asset('event_messages/') ?>/' + group_id,
                                        "chat_id": result.chat_id,
                                        "chat_type": 'g',
                                        "chat_type_id": group_id,
                                        "to_be_show": 'g',
...
<?php if (Auth::user()) { ?>
    <!-- Message modal Start -->
    <div class="modal fade" id="friend_group_messages" tabindex="-1" role="dialog" aria-labelledby="add_affiliation"
         aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title text-black" id="exampleModalLabel">New message To Event Group <span
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-group">
                                    <select type="text" name="friends[]" class="form-control"
                                            ...
                                            multiple="multiple">
                                        <?php
                                        if (!empty($group->approvedMembers)) {
                                            foreach ($group->approvedMembers as $key => $member) {
                                                if ($member->getMemberDetail->id != $current_id) {
                                                    ?>
                                                    <option <?= ($key == '0') ? 'selected=""' : '' ?>
                                                            value="<?= $member->getMemberDetail->id ?>"> <?= $member->getMemberDetail->first_name . ' ' . $member->getMemberDetail->last_name ?> </option>
                                                    <?php
                                                }
                                            }
                                        }
...
<!-- attach file in event group messages            -->
<script>
    files = '';
