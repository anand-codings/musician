                                        ...
                                        foreach ($chats_data as $chat) {
                                            if ($chat->chat_type == 's') {
                                                ...
                                                if ($other_user->is_online == 1) {
                                                    ...
                                                    ?>
                                                    <li id="check_if_online_<?= $other_user->id ?>" onclick="getOtherChat('chat_id_<?= $chat->id ?>', this, '<?= $chat->id ?>', '<?= $name ?>', '<?= $other_user->id ?>', '<?= $profile_url ?>', '<?php echo $other_image ?>', '0', '10', '<?= $other_user->is_active ?>', 's', '<?= $studio->id ?>')"><span class="bg_image_round user_active_status" style="background-image: url(<?php echo $other_image ?>) "><span class="active"></span></span></li>
                                                    <?php
                                                }
                                            }
                                        }
                                        ...
                                        foreach ($chats_data as $chat) {
                                            ...
                                            <li data-id="<?= $chat->id ?>" id="single_chat_user<?= $chat->id ?>" class="chat_user_listing <?php if ($i == 1) { ?> active <?php } ?>" >
                                                <div class="list_outer_wrap d-flex">
                                                    ...
                                                    <a href="#" onclick="getOtherChat('chat_id_<?= $chat->id ?>', this, '<?= $chat->id ?>', '<?= $name ?>', '<?= $other_user->id ?>', '<?= $profile_url ?>', '<?php echo $other_image ?>', '0', '10', '<?= $other_user->is_active ?>', , 's', '<?= $studio->id ?>')">
                                                        <div class="d-flex">
                                                            <div>
                                            ...
                                            if ($message->sender_id == $current_id) {
                                                ...
                                                <li class="right" id="single_message<?= $message->id ?>">
                                                    <div class="d-flex flex-row-reverse">
                                                        <figure>
                                                            <?php
                                                            if ($message->message_type == 'g' && $message->studio->admin_id == $current_id) {
                                                                ...
                                                                ?>  
                                                                <span class="bg_image_round" onclick="location.href = '<?= asset('teaching_studio_time_line/' . $message->studio_id) ?>'" style="background-image: url(<?php echo $pic ?>)"></span>
                                                                <?php
                                                                ...
                                                                ?>
                                                                <span class="bg_image_round" onclick="location.href = '<?= asset('accompanist_time_line/' . $message->accompanist->id) ?>'" style="background-image: url(<?php echo $pic ?>)"></span>
                                                            <?php } else { ?>
                                                ...
                                                if ($message->message_type == 'g' && $message->group->admin_id == $other_message_user->id) {
...
//        testing = ScrollHeight[0].mcs.content.height();
        ...
        socket.on('message_send', function (data) {
            if (data.user_id == current_id && other_id == data.other_id && data.to_be_show == 's') {
                other_id = $('#other_user_chat_id').val();
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data) {
                            ...
                            //                            setTimeout(function () {
                            ////                                skip = parseInt(skip) + parseInt(10);
                            ////                                getOtherChat(id, 'ajax', chat_id, other_name, other_id, url, image, skip, 5);
                            ////                                $('.chat_box_wrapper').mCustomScrollbar("update");
                            ////                            $('.chat_box_wrapper').mCustomScrollbar("scrollTo", 'bottom');
                            ////                                  return true;
                            //                            }, 2000);
                        } else {
