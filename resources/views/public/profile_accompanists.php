            ...
            <div class="container lg-fluid-container">
                <div class="row">
                    ...
                    <div class="col-lg-9 col-md-12">
                        ...
                        <h4 class="font-weight-bold text_darkblue text-uppercase mb-4"> Accompanists</h4>
...
<script>
    ...
    function loadTeachingStudios(skip, take, isScroll) {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo asset('fetch_profile_accompanists/'); ?>",
    ...
    function addBookmark(accompanist_id) {
        $.ajax({
            type: "POST",
            url: "<?= asset('bookmark_accompanist'); ?>",
            data: {accompanist_id: accompanist_id, is_bookmarked: 1, "_token": '<?= csrf_token() ?>'},
            success: function (response) {
                if (response.message == 'success') {
                    $("#add-bookmark-btn-" + accompanist_id).css("display", "none");
                    $("#remove-bookmark-btn-" + accompanist_id).css("display", "block");
                } else {
    ...
    function removeBookmark(accompanist_id) {
        $.ajax({
            type: "POST",
            url: "<?= asset('bookmark_accompanist'); ?>",
            data: {accompanist_id: accompanist_id, is_bookmarked: 0, "_token": '<?= csrf_token() ?>'},
            success: function (response) {
                if (response.message == 'success') {
                    $("#add-bookmark-btn-" + accompanist_id).css("display", "block");
                    $("#remove-bookmark-btn-" + accompanist_id).css("display", "none");
                } else {
                    $('#showError').html('Please Try Again').fadeIn().fadeOut(5000);
                }
            }
