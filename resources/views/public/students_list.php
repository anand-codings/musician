                                ...
                                <div class="collaborative-row">
                                    <div class="collaborative-header">
                                        <div class="colaborative-header-row">
                                            <h5>Students</h5>
                                            <div>
                                            <button class="btn btn_aqua btn-square font-weight-normal" data-target="#student_group_messages_modal" data-toggle="modal"> <i class="fas fa-plus"></i> New Message</button>
                                            <button class="btn btn_aqua btn-square font-weight-normal" data-target="#refer_student_modal" data-toggle="modal"> <i class="fas fa-plus"></i> Add Friends</button>
                                            </div>
                                        </div>
                                    </div>
                                    <?php
//                                if (Auth::user()) {
                                    if (!empty($studio->members)) {
                                        foreach ($studio->members as $key => $member) {
                                           
                                            ?>
                                            <div class="collaborative-body-row">
                                                <div class="collaborative-body-column">
                                                    <div class="friends-image" style="background-image:url('<?php echo asset('public/images/' . $member->getMemberDetail->photo); ?>')"></div>
                                                    <a href="<?php echo asset('/profile_timeline/' . $member->getMemberDetail->id); ?>"><p><?php echo $member->getMemberDetail->first_name . ' ' . $member->getMemberDetail->last_name; ?></p></a>
                                                </div>
                                                <div class="collaborative-body-column1">
                                                    <?php if($member->getMemberDetail->id != $current_id){
                                                    ?>
                                                    <a href="javascript:void(0)"  class="btn btn-white-outline" data-toggle="modal" data-target="#modal_search_messages<?= $member->getMemberDetail->id ?>"  > <svg xmlns="http://www.w3.org/2000/svg" width="20" height="17" viewBox="0 0 20 17">
                                            ...
                                            <?php if($member->getMemberDetail->id != $current_id){
                                                    ?>
                                            <!-- Message modal Start -->
                                            <div class="modal fade" id="modal_search_messages<?= $member->getMemberDetail->id; ?>" tabindex="-1" role="dialog" aria-labelledby="add_affiliation" aria-hidden="true">
                                                <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h6 class="modal-title text-black" id="exampleModalLabel">New message To <span class="text_maroon"> <?= $member->getMemberDetail->first_name . ' ' . $member->getMemberDetail->last_name; ?> </span></h6>
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-12">
                                                                        <div class="form-group mb-0">
                                                                            <textarea id="message<?= $member->getMemberDetail->id; ?>" class="form-control h_140" placeholder="Write a message"></textarea>
                                                                        </div>
                                                                    </div>
                                                                </div> <!-- row -->
                                                                <div class="mt-2">
                                                                    <button type="button" onclick="sendMessage('<?= $member->getMemberDetail->id; ?>')" class="btn btn-gradient btn-xl text-semibold">Send</button>
                                                                </div>
                                                            </form>                        
                                                        </div> <!-- modal-body-->
                                                    </div> <!-- modal-content-->
...
//                                }
        ...
        <!-- Message modal Start -->
        <div class="modal fade" id="refer_student_modal" tabindex="-1" role="dialog" aria-labelledby="add_affiliation" aria-hidden="true">
            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="teaher-header">
                            <h6 class="modal-title text-black" id="exampleModalLabel">Add more Students</h6>
                    ...
                    <div class="modal-body">
                        <form>
                                ...
                                <div class="col-12">
                                    <div class="teacher-box">
                                        <div class="techer-header">
                                            <input type="text" class="form-control" id="searchContent" onkeyup="searchStudentFromList();" placeholder="Search "/>
                                        </div>
                                        <div class="teacher-lists">
                                            <div class="collaborative_container" id="friend_list">

                                                <?php
                                                if ((Auth::user() && (!empty($studio->members)))) {
                                                            ...
                                                            <div class="collaborative-body-row">
                                                                ...
                                                                <div class="collaborative-body-column1">
                                                                    <label class="checkbox-main">
                                                                        <input type="checkbox" id="members_<?= $friend->getFriendDetail->id ?>" class="member_list"   value="<?= $friend->getFriendDetail->id ?>" name="friendlist">
                            ...
                            <div class="mt-2">
                                <button <?= count($friends)==0 ? 'disabled': '' ?> type="button" id="add_friends" onclick="referStudentToJoin(<?= $studio->id?> , 'user')"   class="btn btn-round btn-gradient btn-xl text-semibold">Add Now</button>
                            </div>
                        </form>                        
                    </div> <!-- modal-body-->
                </div> <!-- modal-content-->
         ...
         <!-- student group messages modal -->
        <?php if (Auth::user()) { ?>
        <!-- Message modal Start -->
        <div class="modal fade" id="student_group_messages_modal" tabindex="-1" role="dialog" aria-labelledby="add_affiliation" aria-hidden="true">
                        ...
                        <form>
                            <div class="row freinds_message">
                                <div class="col-12">
                                    ...
                                    <div class="form-group">
                                        <select type="text" name="friends[]" class="form-control" id="group_bulk_messages<?= $current_id ?>" style="width: 100%" multiple="multiple">
                                            <?php
                                            if (!empty($studio->members)) {
                                                
                                                foreach ($studio->members as $key => $student) {
                                                    if($student->getMemberDetail->id != $current_id)
                                                    {
                                                    ?>
                                                    <option <?= ($key == '1') ? 'selected=""' : '' ?> value="<?= $student->getMemberDetail->id ?>"> <?= $student->getMemberDetail->first_name . ' ' . $student->getMemberDetail->last_name ?> </option>
                                                    <?php
                                                    }
                                                }
                                            }
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea id="groupMessage<?= $current_id ?>" class="form-control h_140" placeholder="Write a message"></textarea>
                                        <div class="font-14 text-danger text-right"></div>
                                    </div>
                            ...
                            <div class="mt-2 text-center">
                                <button type="button" onclick="sendMessageToStudents('<?= $current_id ?>')" class="btn btn-gradient btn-round btn-xl text-semibold">Send</button>
                            </div>
                        </form>
                    </div> <!-- modal-body-->
                </div> <!-- modal-content-->
    ...
    <?php } ?>
        ...
        <!-- send message to studio students function  -->
        <?php if ($current_user) { ?>
        <script>
            ...
            function sendMessageToStudents(otherid) {
                    ...
                    if (/\S/.test(message)) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                $('#student_group_messages_modal').modal('hide');
                                //empty modal fields 
                                ...
                                $.each(recivers, function (index, item) {
                                    ...
                                    socket.emit('groupmessage_get', {
                                        ...
                                        socket.emit('notification_get', {
                                            ...
                                            "url": '<?= asset('student_studio_messages') ?>/'+ type_id+ '?chat_studio_id='+result.chat_id,
                                            "chat_id": result.chat_id,
                                            "chat_type": 's',
...
//                $("#tiny-icon-spacer").show();
       ...
       <script>
function searchStudentFromList() {
        ...
        <script>
           ...
           function referStudentToJoin(studio_id, studio_type)
           {
               ...
               if(!member_ids.length)
               {
                ...
                $.ajax({
                type:'post',
                url:"<?php echo asset('refer_student_to_join_studio'); ?>",
                data:data,
                processData:false,
                contentType:false,
                success:function(data)
                {
                    $('#refer_student_modal').modal('hide');
...
//                                    $('.').append('<div class="collaborative-body-row accompanist-member-'+notification.notification_for_admin.on_user+'"><div class="collaborative-body-column"><div class="friends-image" style="background-image:url('<?php // echo asset('public/images/'); ?>''+notification.notification_for_admin.on_user+')"></div><a href="<?php echo asset('/profile_timeline/'); ?>'+notification.notification_for_admin.on_user+'"><p><?php // echo $member->getMemberDetail->first_name . ' ' . $member->getMemberDetail->last_name; ?></p></a></div>');
                                    $('.students_list_tab_<?= $studio->id ?>').children('ul').prepend('<li id="studio-student-' + notification.notification_for_admin.on_user + '"><a href="<?php echo asset('/profile_timeline/') ?>/' + notification.notification_for_admin.on_user + '"><img src="<?php echo asset('public/images/') ?>/' + notification.user_photo + '"><div class="friends_name"><h6>'+notification.user_fname + ' ' + notification.user_lname+'</h6></div></a></li>');

                                // dynamic append student in teaching studio when admin add students in studio
                                ...
                                //show follow message
                                $('#showSuccess').html("Successfully added friends as a student in your  <?php echo $studio->name ?> teaching studio ").fadeIn().fadeOut(5000);
                            }
                  }
                  else if(data.error !='')
                  {
