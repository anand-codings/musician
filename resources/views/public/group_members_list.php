                            ...
                            <div class="colaborative-header-row">
                                ...
                                <div>
                                    <button class="btn btn_aqua btn-square font-weight-normal"
                                            data-target="#group_messages_modal" data-toggle="modal"><i
                                                class="fas fa-plus"></i> New Message
                                    </button>
                                    <button class="btn btn_aqua btn-square font-weight-normal"
                                            data-target="#refer_to_join_group_modal" data-toggle="modal"><i
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h6 class="modal-title text-black" id="exampleModalLabel">New
                                                        message To
                                                        <span class="text_maroon"> <?= $member->getMemberDetail->first_name . ' ' . $member->getMemberDetail->last_name; ?> </span>
                                                    </h6>
                            ...
                            $.each(recivers, function (index, item) {
                                ...
                                if (item != <?=$current_id?>) {
                                    socket.emit('notification_get', {
                                        ...
                                        "url": '<?= asset('event_messages/'.$group->id) ?>',
                                        "chat_id": result.chat_id,
                                        "chat_type": 'g',
                                        "chat_type_id": result.chat_id,
                                        "to_be_show": 'g',
...
<!-- group messages modal for event services -->
<?php if (Auth::user()) { ?>
    <!-- Message modal Start -->
    <div class="modal fade" id="group_messages_modal" tabindex="-1" role="dialog" aria-labelledby="add_affiliation"
         aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title text-black" id="exampleModalLabel">New Event Group To <span
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-group">
                                    <select type="text" name="friends[]" class="form-control"
                                        ...
                                        <?php
                                        if (!empty($group->approvedMembers)) {
                                            foreach ($group->approvedMembers as $key => $friend) {
                                                if ($friend->getMemberDetail->id != $current_id) {
                                                    ?>
...
<?php if (Auth::user()) { ?>
    <!-- Message modal Start -->
    <div class="modal fade" id="_messages_modal" tabindex="-1" role="dialog" aria-labelledby="add_affiliation"
         aria-hidden="true">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="bulk-messages3">
                                        ...
                                        if (Auth::user()) {
                                            if (!empty($group->approvedMembers)) {
                                                foreach ($group->approvedMembers as $key => $teacher) {
                                                    ?>
                                ...
                                <div class="form-group ">
                                    <div class="attact-link btn-file">
                                        ...
                                        <input type="file" id="attachment_member_files" id="link-file"/>
                                    </div>
                                </div>
...
<!-- Message modal Start -->
<div class="modal fade" id="refer_to_join_group_modal" tabindex="-1" role="dialog" aria-labelledby="add_affiliation"
     aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <div class="teaher-header">
                    <h6 class="modal-title text-black" id="exampleModalLabel">Add More Friends</h6>
                    <button <?= count($friends) == 0 ? 'disabled' : '' ?> type="button" id="select_list"
                                                                          class="btn btn-round btn_aqua">Select All
                    </button>
                </div>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="fas fa-times-circle"></i>
                ...
                <form>
                            ...
                            <div class="teacher-box">
                                ...
                                <div class="teacher-lists">
                                    <div class="collaborative_container" id="friend_list">
                                        ...
                                        <?php
                                        if ((!empty($friends)) && ($group->admin_id != '')) {
                                            foreach ($friends as $key => $friend) {
                                                if ($group->admin_id != $friend->getFriendDetail->id) {
                                                    ?>
                                                    <div class="collaborative-body-row">
                                                        <div class="collaborative-body-column">
                                                            <div class="friends-image"
                    ...
                    <div class="mt-2">
                        <button <?= count($friends) == 0 ? 'disabled' : '' ?> type="button" id="add_friends"
                                                                              onclick="referToJoin(<?= $group->id ?> , 'group')"
...
<script>
    $(document).ready(function () {
        event_members_count = parseInt($('#event_members_count').html(), 10);
    });

    function referToJoin(group_id, group_type) {
        ...
        $.ajax({
            type: 'post',
            url: "<?php echo asset('refer_friend_to_join_group'); ?>",
            data: data,
            processData: false,
            contentType: false,
            success: function (data) {
                $('#refer_to_join_group_modal').modal('hide');
                if ((data.success != '') && (data.notification != '')) {
                    if (<?=$current_id?> != <?=$group->admin_id?>)
                    {
                        ...
                        //show follow message
                        $('#showSuccess').html(data.success).fadeIn().fadeOut(5000);
                        $('#add_friends').attr('disabled', true);
                ...
                else
                    {
                        ...
                        $.each(data.notification, function (index, notification) {
                            ...
                            event_members_count += 1;
                            $('.members_tab_<?= $group->id ?>').children('ul').prepend('<li id="member-' + notification.notification_for_admin.on_user + '"><a href="<?php echo asset('/profile_timeline/') ?>/' + notification.notification_for_admin.on_user + '"><img src="<?php echo asset('public/images/') ?>/' + notification.user_photo + '"></a></li>');
                        ...
                        $('#event_members_count').html(event_members_count);
                        //show follow message
                        $('#showSuccess').html('Successfully added Friends to Event!').fadeIn().fadeOut(5000);
                    }
                } else if (data.error != '') {
                    $('#refer_to_join_group_modal').modal('hide');
...
<!-- attach file in group member list            -->
<script>
    files = '';
