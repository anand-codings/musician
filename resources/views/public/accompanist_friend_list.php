                        ...
                        <div class="collaborative-row">
                            <div class="collaborative-header">
                                <div class="colaborative-header-row">
                                    <h5>Accompanist Members</h5>
                                    <div>
                                    <button class="btn btn_aqua btn-square font-weight-normal"
                                            data-target="#accompanist_group_messages_modal" data-toggle="modal"><i
                                                class="fas fa-plus"></i> New Message
                                    </button>
                                    <button class="btn btn_aqua btn-square font-weight-normal"
                                            data-target="#refer_to_join_accompanist_modal" data-toggle="modal"><i
                                                class="fas fa-plus"></i> Add Friends
                                    </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            if (Auth::user()) {
                                ...
                                if (($accompanist->members->count())) {
                                    foreach ($accompanist->members as $key => $member) {
                                        if ($member->is_approved == '1') {
                                            ?>
                                            <div class="collaborative-body-row accompanist-member-<?= $member->getMemberDetail->id ?>">
                                            ...
                                            <?php if ($member->getMemberDetail->id != $current_id) { ?>
                                                ...
                                                <div class="modal fade"
                                                     id="modal_search_messages<?= $member->getMemberDetail->id; ?>"
                                                     tabindex="-1" role="dialog" aria-labelledby="add_affiliation"
                                                     aria-hidden="true">
                                                    <div class="modal-dialog modal-sm modal-dialog-centered"
                                                         role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h6 class="modal-title text-black"
                                                                    id="exampleModalLabel">New message To <span
                                                                ...
                                                                <form>
                                                                    ...
                                                                    <div class="mt-2">
                                                                        <button type="button"
                                                                                ...
                                                                                class="btn btn-gradient btn-xl text-semibold">
                                                                            Send
                                                                        </button>
                                                                    </div>
                                                                </form>
                                    ...
                                    ?>
                                    <div class="collaborative-body-row" id="empty_row">
                                        <div class="collaborative-body-column d-flex justify-content-center">
                                            No Friend Found
                                        </div>
...
<!-- send group message to to memebers in accompanist -->
<?php if ($current_user) { ?>
    <script>
        function sendMessages(otherid) {

            var recivers_array = [];
            var message = $('#groupMessage' + otherid).val();
            if (message) {
                recivers_array = $('#group_bulk_messages' + otherid).val();
                ;
                counter = 0;

                var message_type = 'g';
                var type_id = '<?= $accompanist->id ?>';
                var data = new FormData();
                data.append('group_message', message);
                ...
                if (/\S/.test(message)) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            $('#group_messages_modal').modal('hide');
                            counter++;
                            if (counter == 1) {
...
<?php if ($current_user) { ?>
    <script>
        function sendGroupMessages(otherid) {
            //                var file = $('#imgInp').val();
            var recivers_array = [];
            ...
            if (message) {
                ...
                if (/\S/.test(message)) {
                    ...
                    $.ajax({
                        ...
                        contentType: false,
                        success: function (data) {
                            $('#accompanist_group_messages_modal').modal('hide');
                            counter++;
                            if (counter == 1) {
                                $('#showSuccess').html('Message Send Successfully !').fadeIn().fadeOut(5000);
                            }
                            ...
                            $.each(recivers, function (index, item) {
                                ...
                                if (item != <?= $current_id ?>) {
                                    socket.emit('notification_get', {
                                        ...
                                        "other_name": '<?php echo $current_user->first_name . ' ' . $current_user->last_name; ?>',
                                        "photo": '<?php echo $current_photo; ?>',
                                        "text": '<?php echo $current_user->first_name . ' ' . $current_user->last_name; ?>' + ' added you in event group chat ' + title,
                                        "url": '<?= asset('accompanist_messages/'.$accompanist->id) ?>',
                                        "chat_id": result.chat_id,
                                        "chat_type": 'a',
                                        "chat_type_id": result.chat_id,
                                        "to_be_show": 'a',
...
<!-- accompanist group messages modal -->
<?php if (Auth::user()) { ?>
    <!-- Message modal Start -->
    <div class="modal fade" id="accompanist_group_messages_modal" tabindex="-1" role="dialog"
         aria-labelledby="add_affiliation" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title text-black" id="exampleModalLabel">New Accompanist Chat Group <span
                ...
                <div class="modal-body">
                    <form>
                                ...
                                <div class="form-group">
                                    <select type="text" name="friends[]" class="form-control"
                                        ...
                                        if (!empty($accompanist->members)) {
                                            foreach ($accompanist->members as $key => $friend) {
                                                if ($friend->getMemberDetail->id != $current_id && $friend->is_approved == '1') {
                                                    ?>
                                                    <option <?= ($key == '0') ? 'selected=""' : '' ?>
                                                            value="<?= $friend->getMemberDetail->id ?>"> <?= $friend->getMemberDetail->first_name . ' ' . $friend->getMemberDetail->last_name ?> </option>
                                                    <?php
                                                }
                                            }
                                        }
                        ...
                        <div class="mt-2 text-center">
                            <button type="button" onclick="sendGroupMessages('<?= $current_id ?>')"
...
<!-- Message modal Start -->
<div class="modal fade" id="refer_to_join_accompanist_modal" tabindex="-1" role="dialog"
     aria-labelledby="add_affiliation" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <div class="teaher-header">
                    <h6 class="modal-title text-black" id="exampleModalLabel">Add More Accompanist</h6>
                    <button type="button" id="select_list" class="btn btn-round btn_aqua">Select All</button>
                </div>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="fas fa-times-circle"></i>
                </button>
            ...
            <div class="modal-body">
                <form>
                            ...
                            <div class="teacher-box">
                                ...
                                <div class="teacher-lists">
                                    <div class="collaborative_container" id="friend_list">
                                        ...
                                        <?php
                                        if ((!empty($friends)) && ($accompanist->admin_id != '')) {
                                            foreach ($friends as $key => $friend) {
                                                if ($accompanist->admin_id != $friend->getFriendDetail->id) {
                                                    ?>
                    ...
                    <div class="mt-2">
                            ...
                            <button <?= count($friends)==0 ? 'disabled': '' ?> type="button" id="add_friends"
                                    onclick="referToJoin(<?= $accompanist->id ?>, 'member')"
...
<script>
    $(document).ready(function () {
        accompanist_members_count = parseInt($('#accompanist_members_count').html(), 10);
    });
    var clicked = false;
    ...
    function referToJoin(accompanist_id, accompanist_type) {
        var member_ids = [];
        ...
        data.append('accompanist_type', accompanist_type);
        data.append('member_ids', member_ids);
        data.append('_token', '<?= csrf_token() ?>');
        $.ajax({
            type: 'post',
            url: "<?php echo asset('refer_friend_to_join_accompanist'); ?>",
            data: data,
            processData: false,
            contentType: false,
            success: function (data) {
                $('#refer_to_join_accompanist_modal').modal('hide');
                if ((data.success != '') && (data.notification != '')) {

                    if (<?= $current_id ?> != <?= $accompanist->admin_id ?>)
                    {

                        $.each(data.notification, function (index, notification) {

                            socket.emit('notification_get', {
                                "other_id": notification.notification_for_admin.on_user,
                                "user_id": notification.notification_for_admin.user_id,
                        ...
                        //show follow message
                        $('#showSuccess').html('Successfully send request to Accompanist admin!').fadeIn().fadeOut(5000);
...
<!-- attach file in accompanist create group chat -->
<script>
    files = '';
