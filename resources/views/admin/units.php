                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach($units as $unit) { ?>
                                                <tr>
                                                    <td><?php echo $i; $i++; ?></td>
                                                    <td><?=$unit->title?></td>
                                                    <td><a href="javascript:void(0)" onclick="deleteUnit(<?=$unit->id?>)"  class="text-danger delete"><i class="fa fa-trash-o"></i></a></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
...
<script>
    function deleteUnit(unit_id){
        if(confirm('Are you sure that you want to delete this unit?')){
            $.ajax({
                url: base_url + 'delete_unit_admin',
                type: 'POST',
                ...
                success:function(data) {
                    console.log(data);
                    window.location.href= base_url+'units_admin';
                }
            });
        }
    }
